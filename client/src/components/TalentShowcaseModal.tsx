...
const TalentShowcaseModal: React.FC<TalentShowcaseModalProps> = ({
  ...
  useEffect(() => {
    if (open && talent) {
      setLoading(true);
      // Preload immediate fallback to ensure instant rendering without blank screen
      setProfileData({
        ...talent,
        deliveredWorkflows: DEFAULT_WORKFLOWS,
        links: {
      ...
      setActiveVideoUrl(DEFAULT_WORKFLOWS[0].demoVideoUrl);

      // Fetch dynamic profile from backend slice
      profileApi.getById(talent.id)
        ...
        .catch(() => {
          // Keep current fallback smoothly
        })
